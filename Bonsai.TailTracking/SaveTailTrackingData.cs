...
namespace Bonsai.TailTracking
{
    ...
    public class SaveTailTrackingData : FileSink<object, TailTrackingDataDisposable>
    {
        ...
        public SaveTailTrackingData()
        {
            Overwrite = false;
        }

        protected override TailTrackingDataDisposable CreateWriter(string fileName, object input)
        {
            ...
            lock (SyncRoot)
            {
                ...
                return new TailTrackingDataDisposable(writer, Disposable.Create(() =>
                {
                    lock (SyncRoot)
                    {
                        writer.Close();
        ...
        protected override void Write(TailTrackingDataDisposable writer, object input)
        {
            try
            {
                string saveData = "";
